package  eTook_Engin
{
	...
	/**
	 * A tool for tracking objects in memory and seeing if they've been properly deleted. The layout is described
	 * in http://www.craftymind.com/2008/04/09/kick-starting-the-garbage-collector-in-actionscript-3-with-air/
	...
	public class MemoryTracker
	{
		...
		/**
		 * Start garbage collection and check for any references that remain. This will only
		 * work in the debug player or AIR app (as it's the only place we can call System.gc()).
		 * This will work over a numbr of frames
		 */
		...
		// Perform a garbage collect. We call it a number of times as the first time is for the
		// mark phase, while the second call performs the sweep
		private static function _gc( e:Event ):void
		{
			...
			// In debug mode, we call System.gc() a total of 4 times: 3 in this function, and
			// one final time in _doLastGC()
			if ( runLast )
			{
				// use e.target in the rare chance that MemoryTracker.stage has been cleared in the meantime
				( e.target as Stage ).removeEventListener( Event.ENTER_FRAME, MemoryTracker._gc );
				setTimeout( MemoryTracker._doLastGC, 40 );
			}
		}
		
		// Performs the last garbage collect. This is from the link at the top: "Lastly, not all
		// features in AIR could be unhooked with our enterFrame trick, after another couple days
		// of testing we found components that needed to be unhooked with Timers like the HTML
		// component."
